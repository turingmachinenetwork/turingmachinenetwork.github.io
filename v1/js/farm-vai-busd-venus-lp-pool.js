...
const FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ADDR = '0x5bf342bb704d55fDfC4c9B2E2fA1292f543dC8dc';
const TURING_VAI_BUSD_VENUS_LP_TOKEN_ADDR = '0xe2EAE89F05583FBef95408f04f6CC12b78FdBDcb';
let farmTuringPoolContract;
let userDataInFarmTuringPool = {};
const FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ABI = [
	{
		"inputs": [
			{
...
function initFarmPoolContract() {
	if (!farmTuringPoolContract) {
		...
		farmTuringPoolContract = new _web3.eth.Contract(FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ABI, FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ADDR);
	}
	setTimeout(function(){ 
		initFarmPoolContract();
	}, 3000);
...
function loadData() {
	...
	function _drawUI() {
		...
		$('.farm-turing-vai-busd-venus-lp-pool-your-stake').html(formatBalance(userDataInFarmTuringPool.userWantShare, 2));
		$('.farm-turing-vai-busd-venus-lp-pool-your-earned').html(formatBalance(userDataInFarmTuringPool.userTuringPending, 6));
		$('.farm-turing-vai-busd-venus-lp-pool-total-supply').html(`$${formatBalance(userDataInFarmTuringPool.tvl * turingLPPrice, 2)}`);
		$('.farm-turing-vai-busd-venus-lp-pool-user-turing-lp-bal').html(numberWithCommas(userDataInFarmTuringPool.userWantBal, 2));
		$('.farm-turing-vai-busd-venus-lp-pool-user-turing-lp-state').html(numberWithCommas(userDataInFarmTuringPool.userWantShare, 2));
		$('.farm-turing-vai-busd-venus-lp-pool-apy').html(`${formatBalance(_poolAPY, 2)}%`);
		$('.farm-turing-vai-busd-venus-lp-pool-apy-turing').html(`${formatBalance(userDataInFarmTuringPool.turingRewardAPY, 2)}%`);
		$('.farm-turing-vai-busd-venus-lp-pool-apy-venus').html(`${formatBalance(_venusAPY, 2)}%`);
		$('.farm-turing-vai-busd-venus-lp-pool-apy-trade').html(`${formatBalance(userDataInFarmTuringPool.tradeAPY, 2)}%`);
...
function initUserAction() {
	...
	function _clickMaxDeposit() {
		$('.farm-turing-vai-busd-venus-lp-pool-max-turing-lp-deposit').click(function(e) {
			e.preventDefault();

			let amount = userDataInFarmTuringPool.userWantBal ? userDataInFarmTuringPool.userWantBal : 0;
			
			$('input[type=number][name=farm_turing_vai_busd_venus_lp_pool_deposit_turing_lp_amt]').val(amount);
		});
	}

	function _clickMaxWithdraw() {
		$('.farm-turing-vai-busd-venus-lp-pool-max-turing-lp-withdraw').click(function(e) {
			e.preventDefault();

			let amount = userDataInFarmTuringPool.userWantShare ? userDataInFarmTuringPool.userWantShare : 0;
			$('input[type=number][name=farm_turing_vai_busd_venus_lp_pool_withdraw_turing_lp_amt]').val(amount);
		});
	}
	function _approve() {
		$('.btn-approve-farm-turing-vai-busd-venus-lp').click(function(e) {
			e.preventDefault();
			let userAddr = getCurrentAddress();
			if (!userAddr) {
				return false;
			...
			let _token = new web3.eth.Contract(TOKEN_ABI, TURING_VAI_BUSD_VENUS_LP_TOKEN_ADDR);
			_token
				.methods
				.approve(FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ADDR, ALLOW_LIMIT_AMT)
	...
	function _deposit() {
		$('.btn-deposit-farm-turing-vai-busd-venus-lp').click(function(e) {
			e.preventDefault();
			let amount = $('input[type=number][name=farm_turing_vai_busd_venus_lp_pool_deposit_turing_lp_amt]').val();
			...
			let _transactionHistory = {}; 
			let _token = new web3.eth.Contract(TOKEN_ABI, TURING_VAI_BUSD_VENUS_LP_TOKEN_ADDR);
			let farmTuringPoolContract = new web3.eth.Contract(FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ABI, FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ADDR);

			_token
				.methods
				.allowance(userAddr, FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ADDR)
				.call()
				.then(amountAllow => {
			...
			function _approvelToken() {
				...
				_token.methods.approve(FARM_TURING_VAI_BUSD_VENUS_LP_POOL_CONTRACT_ADDR, ALLOW_LIMIT_AMT).send({ from: getCurrentAddress() })
					.on('transactionHash', function(hash) {
	                	_showPopup('confirm-popup');
	                })
	                .on('confirmation', function(confirmationNumber, receipt){
			...
			function _deposit() {
				farmTuringPoolContract.methods.deposit(toBN(amount, 18)).send({ from: getCurrentAddress() })
	                ...
	                .on('confirmation', function(confirmationNumber, receipt){
	                    if (receipt.status == true && !_transactionHistory[receipt.transactionHash]) {
							_transactionHistory[receipt.transactionHash] = true;
							$('input[type=number][name=farm_turing_vai_busd_venus_lp_pool_deposit_turing_lp_amt]').val('');
							_hidePopup('confirm-popup', 0);
							_showPopup('success-confirm-popup');
							_hidePopup('success-confirm-popup', 10000);
						} 
	...
	function _withdraw() {
		$('.btn-withdraw-farm-turing-vai-busd-venus-lp').click(function(e) {
			e.preventDefault();
			let amount = $('input[type=number][name=farm_turing_vai_busd_venus_lp_pool_withdraw_turing_lp_amt]').val();
			amount = parseFloat(amount);
			// amount = toBN(amount, 18);
			let userAddr = getCurrentAddress();
			if (!userAddr) {
			...
			farmTuringPoolContract.methods.withdraw(toBN(amount, 18)).send({ from: getCurrentAddress() })
	                ...
	                .on('confirmation', function(confirmationNumber, receipt){
	                     if (receipt.status == true && !_transactionHistory[receipt.transactionHash]) {
							_transactionHistory[receipt.transactionHash] = true;
							$('input[type=number][name=farm_turing_vai_busd_venus_lp_pool_withdraw_turing_lp_amt]').val('');
							_hidePopup('confirm-popup', 0);
							_showPopup('success-confirm-popup');
							_hidePopup('success-confirm-popup', 10000);
						} 
