...
const FARM_CAKE_LP_POOL_CONTRACT_ADDR = '0xc4758e432de36BC3F44f383FCF9A61D0Ce2a9e64';
const CAKE_BNB_LP_TOKEN_ADDR = '0x0eD7e52944161450477ee417DE9Cd3a859b14fD0';
...
function loadData() {
	...
	function _drawUI() {
		...
		$('.farm-cake-bnb-lp-pool-apy').html(`${formatBalance(userDataInFarmTuringPool.wantRewardAPY + userDataInFarmTuringPool.turingRewardAPY, 2)}%`);
		$('.farm-cake-bnb-lp-pool-apy-cake').html(`${formatBalance(userDataInFarmTuringPool.wantRewardAPY, 2)}%`);
		$('.farm-cake-bnb-lp-pool-turing-apy').html(`${formatBalance(userDataInFarmTuringPool.turingRewardAPY, 2)}%`);
...
function initUserAction() {
	...
	function _clickMaxDeposit() {
		$('.farm-cake-bnb-lp-pool-max-cake-lp-deposit').click(function(e) {
			e.preventDefault();

			let amount = userDataInFarmTuringPool.userWantBal ? userDataInFarmTuringPool.userWantBal : 0;
			
			$('input[type=number][name=farm_cake_bnb_lp_pool_deposit_cake_lp_amt]').val(amount);
		});
	}

	function _clickMaxWithdraw() {
		$('.farm-cake-bnb-lp-pool-max-cake-lp-withdraw').click(function(e) {
			e.preventDefault();

			let amount = userDataInFarmTuringPool.userWantShare ? userDataInFarmTuringPool.userWantShare : 0;
			$('input[type=number][name=farm_cake_bnb_lp_pool_withdraw_cake_lp_amt]').val(amount);
		});
	}

	function _approve() {
		$('.btn-approve-farm-cake-bnb-lp').click(function(e) {
			e.preventDefault();
			let userAddr = getCurrentAddress();
			if (!userAddr) {
				return false;
			...
			let _token = new web3.eth.Contract(TOKEN_ABI, CAKE_BNB_LP_TOKEN_ADDR);
			_token
				.methods
				.approve(FARM_CAKE_LP_POOL_CONTRACT_ADDR, ALLOW_LIMIT_AMT)
	...
	function _deposit() {
		$('.btn-deposit-farm-cake-bnb-lp').click(function(e) {
			e.preventDefault();
			let amount = $('input[type=number][name=farm_cake_bnb_lp_pool_deposit_cake_lp_amt]').val();
			...
			let _transactionHistory = {}; 
			let _token = new web3.eth.Contract(TOKEN_ABI, CAKE_BNB_LP_TOKEN_ADDR);
			let _contract = new web3.eth.Contract(FARM_CAKE_LP_POOL_CONTRACT_ABI, FARM_CAKE_LP_POOL_CONTRACT_ADDR);

			_token
				.methods
				.allowance(userAddr, FARM_CAKE_LP_POOL_CONTRACT_ADDR)
				.call()
				.then(amountAllow => {
			...
			function _deposit() {
				_contract
	                ...
	                .on('confirmation', function(confirmationNumber, receipt){
	                    if (receipt.status == true && !_transactionHistory[receipt.transactionHash]) {
							_transactionHistory[receipt.transactionHash] = true;
							$('input[type=number][name=farm_cake_bnb_lp_pool_deposit_cake_lp_amt]').val('');
							_hidePopup('confirm-popup', 0);
							_showPopup('success-confirm-popup');
							_hidePopup('success-confirm-popup', 10000);
						} 
	...
	function _withdraw() {
		$('.btn-withdraw-farm-cake-bnb-lp').click(function(e) {
			e.preventDefault();
			let amount = $('input[type=number][name=farm_cake_bnb_lp_pool_withdraw_cake_lp_amt]').val();
			amount = parseFloat(amount);
			// amount = toBN(amount, 18);
			let userAddr = getCurrentAddress();
			if (!userAddr) {
			...
			_contract
	            ...
	            .on('confirmation', function(confirmationNumber, receipt){
	                if (receipt.status == true && !_transactionHistory[receipt.transactionHash]) {
							_transactionHistory[receipt.transactionHash] = true;
						$('input[type=number][name=farm_cake_bnb_lp_pool_withdraw_cake_lp_amt]').val('');
						_hidePopup('confirm-popup', 0);
						_showPopup('success-confirm-popup');
						_hidePopup('success-confirm-popup', 10000);
					} 
