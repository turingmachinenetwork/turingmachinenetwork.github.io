...
const LP_TOKEN_ADDR = '0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16';
let farmBusdBNBLPPoolContract;
let userDataInFarmBUSDBNBLPPool = {};
let prizeHistory = [];
...
function loadData() {
	...
	function _updateUserData(_result) {
		...
		userDataInFarmBUSDBNBLPPool.miningSpeed = parseInt(_result[0]); 
		userDataInFarmBUSDBNBLPPool.userWantBal = parseFloatNumber(roundDownFloat(parseInt(_result[1]) / 1e18, 1e18), 18);
		userDataInFarmBUSDBNBLPPool.userBNBBal = parseFloatNumber(parseInt(_result[6]) / 1e18, 18);
		userDataInFarmBUSDBNBLPPool.userTuringPending = parseFloatNumber(parseInt(_result[7]) / 1e18, 18);
		userDataInFarmBUSDBNBLPPool.userWantShare = parseFloatNumber(roundDownFloat(parseInt(_result[8]) / 1e18, 1e18), 18);
		userDataInFarmBUSDBNBLPPool.tvl = parseFloatNumber(parseInt(_result[9]) / 1e18, 18);
		userDataInFarmBUSDBNBLPPool.totalMintPerDay = parseFloatNumber(parseInt(_result[4]) / 1e18, 18);
		userDataInFarmBUSDBNBLPPool.totalWantRewardPerDay = parseFloatNumber(parseInt(_result[5]) / 1e18, 18);
		userDataInFarmBUSDBNBLPPool.turingPrice = parseFloatNumber(parseInt(_result[2]) / 1e18, 18);
		userDataInFarmBUSDBNBLPPool.cakePrice = parseFloatNumber(parseInt(_result[10]) / 1e18, 18);
		userDataInFarmBUSDBNBLPPool.userCakePending = parseFloatNumber(parseInt(_result[3]) / 1e18, 18);
		userDataInFarmBUSDBNBLPPool.turingRewardAPY = 0;
		userDataInFarmBUSDBNBLPPool.wantRewardAPY = 0;
		if (userDataInFarmBUSDBNBLPPool.tvl > 0) {
			userDataInFarmBUSDBNBLPPool.turingRewardAPY = userDataInFarmBUSDBNBLPPool.totalMintPerDay * userDataInFarmBUSDBNBLPPool.turingPrice * 36500 / (userDataInFarmBUSDBNBLPPool.tvl * busdBNBLPPrice);
			userDataInFarmBUSDBNBLPPool.wantRewardAPY = userDataInFarmBUSDBNBLPPool.totalWantRewardPerDay * userDataInFarmBUSDBNBLPPool.cakePrice * 36500 / (userDataInFarmBUSDBNBLPPool.tvl * busdBNBLPPrice);	 
	...
	function _drawUI() {
		...
		$('.farm-busd-bnb-lp-pool-your-stake').html(formatBalance(userDataInFarmBUSDBNBLPPool.userWantShare, 2));
		$('.farm-busd-bnb-lp-pool-your-earned').html(`${formatBalance(userDataInFarmBUSDBNBLPPool.userTuringPending, 6)} TURING ${formatBalance(userDataInFarmBUSDBNBLPPool.userCakePending, 6)} CAKE`);
		$('.farm-busd-bnb-lp-pool-total-supply').html(`$${formatBalance(userDataInFarmBUSDBNBLPPool.tvl * busdBNBLPPrice, 2)}`);
		$('.farm-busd-bnb-lp-pool-user-busd-bnb-lp-bal').html(numberWithCommas(userDataInFarmBUSDBNBLPPool.userWantBal, 2));
		$('.farm-busd-bnb-lp-pool-user-busd-bnb-lp-state').html(numberWithCommas(userDataInFarmBUSDBNBLPPool.userWantShare, 2));
		$('.farm-busd-bnb-lp-pool-apy').html(`${formatBalance(userDataInFarmBUSDBNBLPPool.wantRewardAPY + userDataInFarmBUSDBNBLPPool.turingRewardAPY, 2)}%`);
		$('.farm-busd-bnb-lp-pool-apy-cake').html(`${formatBalance(userDataInFarmBUSDBNBLPPool.wantRewardAPY, 2)}%`);
		$('.farm-busd-bnb-lp-pool-turing-apy').html(`${formatBalance(userDataInFarmBUSDBNBLPPool.turingRewardAPY, 2)}%`);
...
function initUserAction() {
	...
	function _clickMaxDeposit() {
		$('.farm-busd-bnb-lp-pool-max-busd-bnb-lp-deposit').click(function(e) {
			e.preventDefault();

			let amount = userDataInFarmBUSDBNBLPPool.userWantBal ? userDataInFarmBUSDBNBLPPool.userWantBal : 0;
			
			$('input[type=number][name=farm_busd_bnb_lp_pool_deposit_busd_bnb_lp_amt]').val(amount);
		});
	}

	function _clickMaxWithdraw() {
		$('.farm-busd-bnb-lp-pool-max-busd-bnb-lp-withdraw').click(function(e) {
			e.preventDefault();

			let amount = userDataInFarmBUSDBNBLPPool.userWantShare ? userDataInFarmBUSDBNBLPPool.userWantShare : 0;
			$('input[type=number][name=farm_busd_bnb_lp_pool_withdraw_busd_bnb_lp_amt]').val(amount);
		});
	}

	function _approve() {
		$('.btn-approve-farm-busd-bnb-lp').click(function(e) {
			e.preventDefault();
			let userAddr = getCurrentAddress();
			if (!userAddr) {
				return false;
			...
			_token
				.methods
				.approve(FARM_BUSD_BNB_LP_POOL_CONTRACT_ADDR, ALLOW_LIMIT_AMT)
	...
	function _deposit() {
		$('.btn-deposit-farm-busd-bnb-lp').click(function(e) {
			e.preventDefault();
			let amount = $('input[type=number][name=farm_busd_bnb_lp_pool_deposit_busd_bnb_lp_amt]').val();
			...
			let _transactionHistory = {}; 
			let _token = new web3.eth.Contract(TOKEN_ABI, LP_TOKEN_ADDR);
			let _contract = new web3.eth.Contract(FARM_TOKEN_LP_ON_CAKE_POOL_CONTRACT_ABI, FARM_BUSD_BNB_LP_POOL_CONTRACT_ADDR);

			_token
				.methods
				.allowance(userAddr, FARM_BUSD_BNB_LP_POOL_CONTRACT_ADDR)
				.call()
				.then(amountAllow => {
			...
			function _deposit() {
				_contract
	                ...
	                .on('confirmation', function(confirmationNumber, receipt){
	                    if (receipt.status == true && !_transactionHistory[receipt.transactionHash]) {
							_transactionHistory[receipt.transactionHash] = true;
							$('input[type=number][name=farm_busd_bnb_lp_pool_deposit_busd_bnb_lp_amt]').val('');
							_hidePopup('confirm-popup', 0);
							_showPopup('success-confirm-popup');
							_hidePopup('success-confirm-popup', 10000);
						} 
	...
	function _withdraw() {
		$('.btn-withdraw-farm-busd-bnb-lp').click(function(e) {
			e.preventDefault();
			let amount = $('input[type=number][name=farm_busd_bnb_lp_pool_withdraw_busd_bnb_lp_amt]').val();
			amount = parseFloat(amount);
			// amount = toBN(amount, 18);
			let userAddr = getCurrentAddress();
			if (!userAddr) {
			...
			_contract
	            ...
	            .on('confirmation', function(confirmationNumber, receipt){
	                if (receipt.status == true && !_transactionHistory[receipt.transactionHash]) {
							_transactionHistory[receipt.transactionHash] = true;
						$('input[type=number][name=farm_busd_bnb_lp_pool_withdraw_busd_bnb_lp_amt]').val('');
						_hidePopup('confirm-popup', 0);
						_showPopup('success-confirm-popup');
						_hidePopup('success-confirm-popup', 10000);
					} 
