$.TURRING_VENUS_POOL_ADDLP = function() {};
$.TURRING_VENUS_POOL_ADDLP.prototype = (function() {
    var setting = {
        chainId: 56
    };
    ...
    return {
        ...
        displayBTN() {
            ...
            if (!_acc || _acc == "") {
                $(`.btn-approve-token`).hide()
                $(`.btn-approve-base`).hide()
                $(`.btn-deposit`).hide()
                $(`.btn-connect-wallet`).show()
            } else {
                $(`.btn-connect-wallet`).hide()
                $(`.btn-approve-token`).show()
                $(`.btn-approve-base`).show()
                $(`.btn-deposit`).show()
            }
            setTimeout(() => {
                self.displayBTN()
        ...
        async onchangePool() {
            ...
            transactionPoolclick.change(function() {
                ...
                if (pool == "usdcbusd") {
                    token.html("USDC")
                    img_token.attr("src", "../assets/images/usdc_logo.png");
                }
                if (pool == "daibusd") {
                    token.html("DAI")
        ...
        getBaseInputFromTokenInput() {
            try {
                ...
                let callContract = this._getTuringVenusSwapContractToReedData(_lp.contract)
        ...
        async addLP() {
            ...
            $(`.btn-deposit`).click((e) => {
                ...
                console.log("base address: ", _baseAddr);
                ...
                let _contract = this._getTuringVenusSwapMainContract(_contractAddr)
                ...
                function _deposit() {
                    ...
                    console.log("minLP", coreHelper.toBN(minLp));
                    console.log("base input: ", coreHelper.toBN(baseInput));
                    console.log("token input: ", coreHelper.toBN(tokenInput));
        ...
        _getTokenReadContracr(_token) {
            let _abi = abiHelper.getTokenABI()
            return bscContractHelper.getReadContract(_token, _abi)
        },
