    ...
    return {
        ...
        initHomeInterface(_pidOfStakingTur) {
            ...
            function _init(_pid, _data) {
                ...
                $(`#price-${_pid}`).html(`$${ coreHelper.formatBalance(_data.price * _data.prize,2) }`);
        ...
        initDashboardInterface(_pidOfStakingTur) {
            ...
            _pools.forEach(item => {
                ...
                if (_data) {
                    ...
                    $(`#total-reward`).html(`${coreHelper.formatBalance(totalReward,2)} TURING`)
                }
                if (item.type == 'turing_stake' && _data) {
                    _dataOfStakingTur.apy += _data.apy;
                    _dataOfStakingTur.totalShare += _data.totalShare;
                ...
                if (_data && item.type == "cake_no_loss_pool") {
            ...
            function _init(_pid, _data) {
                ...
                $(`#price-${_pid}`).html(`$${ coreHelper.formatBalance(_data.price * _data.prize,2) }`);
                $(`#ticket-${_pid}`).html(`${ coreHelper.formatBalance(_data.totalTickets,2) }`);
                $(`#blc-${_pid}`).html(`${ coreHelper.numberWithCommas(_data.uWantShare, 2) }`);
            }
        ...
        initFarmDetailInterface(_pid) {
            ...
            if (_data) {
                ...
                $(`.apy-cake`).html(`${coreHelper.numberWithCommas(_data.wantRewardAPY)}% CAKE`)
                ...
                $(`.u-share`).html(`${ coreHelper.numberWithCommas(_data.uWantShare, 8) }`);
                ...
                $(`.price`).html(`$${ coreHelper.numberWithCommas(_data.price * _data.prize, 2) }`);
                $(`.will-be-awarded`).html(`${timeOfWillBeAwarded.day} DAY ${timeOfWillBeAwarded.hour} HR ${timeOfWillBeAwarded.min} MIN ${timeOfWillBeAwarded.sec} SEC`);
                ...
                onchangeMaxWantBall(`${ coreHelper.numberWithCommas(_data.uWantBal, 8) }`)
                onchangeMaxShare(`${ coreHelper.numberWithCommas(_data.uWantShare, 8) }`)
            ...
            function onchangeMaxWantBall(value) {
                $(`.max-want-bal`).on("click", () => {
                    $(`input[name=deposit_amt]`).val(value)
                })
            }

            function onchangeMaxShare(value) {
                $(`.max-share`).on("click", () => {
                    $(`input[name=withdraw_amt`).val(value)
                })
            }
            setTimeout(function() {
        ...
        initTuringStakingInterface(_pids) {
            ...
            $(`.u-pending-tur`).html(coreHelper.numberWithCommas(_dataOfStakingTur.uTuringPending, 6));
            ...
            $(`.u-total-share`).html(coreHelper.numberWithCommas(_dataOfStakingTur.uWantShare, 6));
            ...
            function _drawInfo(pid, data) {
                $(`.pool-${pid}-u-share`).html(`${coreHelper.numberWithCommas(data.uWantShare, 6)}`);
                $(`.pool-${pid}-u-unstake-time`).html(`${coreHelper.formatDate(data.withdrawTime, 'DD-MMM-YYYY HH:mm:ss')}`);
            }
        ...
        async _initPoolOf(_pool, pid) {
            ...
            if (_pool.type == 'cake_lp_pool') {
                return this._initCakeLPPool(_pool);
            }

            if (_pool.type == 'cake_no_loss_pool') {
                return this._initCakeNoLossPool(_pool);
            }
            if (_pool.type == 'cake_pool') {
                return this._initCakePool(_pool);
            }
        ...
        async _initCakeNoLossPool(_pool) {
            let _user = coreHelper.getUserAccount();
            let _abi = abiHelper.getCakeNoLossPoolABI();
            let _readContract = bscContractHelper.getReadContract(_pool.contract, _abi, setting.chainId);
            let _r = await _readContract.methods.getData(_user).call();
            ...
            _data.price = coreHelper.parseFloatNumber(parseInt(_r[10]) / 1e18, 18);
            _data.turingPrice = coreHelper.parseFloatNumber(parseInt(_r[11]) / 1e18, 18);
        ...
        async _initCakePool(_pool) {
            ...
            _data.uWantBal = coreHelper.parseFloatNumber(coreHelper.roundDownFloat(parseInt(_r[1]) / 1e18, 1e18), 18);
            _data.turingPrice = coreHelper.parseFloatNumber(parseInt(_r[2]) / 1e18, 18);
            _data.price = coreHelper.parseFloatNumber(parseInt(_r[3]) / 1e18, 18);
            _data.totalMintPerDay = coreHelper.parseFloatNumber(parseInt(_r[4]) / 1e18, 18);
            _data.totalWantRewardPerDay = coreHelper.parseFloatNumber(parseInt(_r[5]) / 1e18, 18);
            _data.uBNBBal = coreHelper.parseFloatNumber(parseInt(_r[6]) / 1e18, 18);
            _data.uTuringPending = coreHelper.parseFloatNumber(parseInt(_r[7]) / 1e18, 18);
        ...
        async _initTuringLPPool(_pool) {
            ...
            _data.uBNBBal = coreHelper.parseFloatNumber(parseInt(_r.userBNBBal_) / 1e18, 18);
