...
const LP_TOKEN_ADDR = '0x61EB789d75A95CAa3fF50ed7E47b96c132fEc082';
let farmBtcbBNBLPPoolContract;
let userDataInFarmBTCBBNBLPPool = {};
let prizeHistory = [];
...
function loadData() {
	...
	function _updateUserData(_result) {
		...
		userDataInFarmBTCBBNBLPPool.miningSpeed = parseInt(_result[0]); 
		userDataInFarmBTCBBNBLPPool.userWantBal = parseFloatNumber(roundDownFloat(parseInt(_result[1]) / 1e18, 1e18), 18);
		userDataInFarmBTCBBNBLPPool.userBNBBal = parseFloatNumber(parseInt(_result[6]) / 1e18, 18);
		userDataInFarmBTCBBNBLPPool.userTuringPending = parseFloatNumber(parseInt(_result[7]) / 1e18, 18);
		userDataInFarmBTCBBNBLPPool.userWantShare = parseFloatNumber(roundDownFloat(parseInt(_result[8]) / 1e18, 1e18), 18);
		userDataInFarmBTCBBNBLPPool.tvl = parseFloatNumber(parseInt(_result[9]) / 1e18, 18);
		userDataInFarmBTCBBNBLPPool.totalMintPerDay = parseFloatNumber(parseInt(_result[4]) / 1e18, 18);
		userDataInFarmBTCBBNBLPPool.totalWantRewardPerDay = parseFloatNumber(parseInt(_result[5]) / 1e18, 18);
		userDataInFarmBTCBBNBLPPool.turingPrice = parseFloatNumber(parseInt(_result[2]) / 1e18, 18);
		userDataInFarmBTCBBNBLPPool.cakePrice = parseFloatNumber(parseInt(_result[10]) / 1e18, 18);
		userDataInFarmBTCBBNBLPPool.userCakePending = parseFloatNumber(parseInt(_result[3]) / 1e18, 18);
		userDataInFarmBTCBBNBLPPool.turingRewardAPY = 0;
		userDataInFarmBTCBBNBLPPool.wantRewardAPY = 0;
		if (userDataInFarmBTCBBNBLPPool.tvl > 0) {
			userDataInFarmBTCBBNBLPPool.turingRewardAPY = userDataInFarmBTCBBNBLPPool.totalMintPerDay * userDataInFarmBTCBBNBLPPool.turingPrice * 36500 / (userDataInFarmBTCBBNBLPPool.tvl * btcbBNBLPPrice);
			userDataInFarmBTCBBNBLPPool.wantRewardAPY = userDataInFarmBTCBBNBLPPool.totalWantRewardPerDay * userDataInFarmBTCBBNBLPPool.cakePrice * 36500 / (userDataInFarmBTCBBNBLPPool.tvl * btcbBNBLPPrice);	 
	...
	function _drawUI() {
		...
		$('.farm-btcb-bnb-lp-pool-your-stake').html(formatBalance(userDataInFarmBTCBBNBLPPool.userWantShare, 6));
		$('.farm-btcb-bnb-lp-pool-your-earned').html(`${formatBalance(userDataInFarmBTCBBNBLPPool.userTuringPending, 6)} TURING ${formatBalance(userDataInFarmBTCBBNBLPPool.userCakePending, 6)} CAKE`);
		$('.farm-btcb-bnb-lp-pool-total-supply').html(`$${formatBalance(userDataInFarmBTCBBNBLPPool.tvl * btcbBNBLPPrice, 2)}`);
		$('.farm-btcb-bnb-lp-pool-user-btcb-bnb-lp-bal').html(numberWithCommas(userDataInFarmBTCBBNBLPPool.userWantBal, 6));
		$('.farm-btcb-bnb-lp-pool-user-btcb-bnb-lp-state').html(numberWithCommas(userDataInFarmBTCBBNBLPPool.userWantShare, 6));
		$('.farm-btcb-bnb-lp-pool-apy').html(`${formatBalance(userDataInFarmBTCBBNBLPPool.wantRewardAPY + userDataInFarmBTCBBNBLPPool.turingRewardAPY, 2)}%`);
		$('.farm-btcb-bnb-lp-pool-apy-cake').html(`${formatBalance(userDataInFarmBTCBBNBLPPool.wantRewardAPY, 2)}%`);
		$('.farm-btcb-bnb-lp-pool-turing-apy').html(`${formatBalance(userDataInFarmBTCBBNBLPPool.turingRewardAPY, 2)}%`);
...
function initUserAction() {
	...
	function _clickMaxDeposit() {
		$('.farm-btcb-bnb-lp-pool-max-btcb-bnb-lp-deposit').click(function(e) {
			e.preventDefault();

			let amount = userDataInFarmBTCBBNBLPPool.userWantBal ? userDataInFarmBTCBBNBLPPool.userWantBal : 0;
			
			$('input[type=number][name=farm_btcb_bnb_lp_pool_deposit_btcb_bnb_lp_amt]').val(amount);
		});
	}

	function _clickMaxWithdraw() {
		$('.farm-btcb-bnb-lp-pool-max-btcb-bnb-lp-withdraw').click(function(e) {
			e.preventDefault();

			let amount = userDataInFarmBTCBBNBLPPool.userWantShare ? userDataInFarmBTCBBNBLPPool.userWantShare : 0;
			$('input[type=number][name=farm_btcb_bnb_lp_pool_withdraw_btcb_bnb_lp_amt]').val(amount);
		});
	}

	function _approve() {
		$('.btn-approve-farm-btcb-bnb-lp').click(function(e) {
			e.preventDefault();
			let userAddr = getCurrentAddress();
			if (!userAddr) {
				return false;
			...
			_token
				.methods
				.approve(FARM_BTCB_BNB_LP_POOL_CONTRACT_ADDR, ALLOW_LIMIT_AMT)
	...
	function _deposit() {
		$('.btn-deposit-farm-btcb-bnb-lp').click(function(e) {
			e.preventDefault();
			let amount = $('input[type=number][name=farm_btcb_bnb_lp_pool_deposit_btcb_bnb_lp_amt]').val();
			...
			let _transactionHistory = {}; 
			let _token = new web3.eth.Contract(TOKEN_ABI, LP_TOKEN_ADDR);
			let _contract = new web3.eth.Contract(FARM_TOKEN_LP_ON_CAKE_POOL_CONTRACT_ABI, FARM_BTCB_BNB_LP_POOL_CONTRACT_ADDR);

			_token
				.methods
				.allowance(userAddr, FARM_BTCB_BNB_LP_POOL_CONTRACT_ADDR)
				.call()
				.then(amountAllow => {
			...
			function _deposit() {
				_contract
	                ...
	                .on('confirmation', function(confirmationNumber, receipt){
	                    if (receipt.status == true && !_transactionHistory[receipt.transactionHash]) {
							_transactionHistory[receipt.transactionHash] = true;
							$('input[type=number][name=farm_btcb_bnb_lp_pool_deposit_btcb_bnb_lp_amt]').val('');
							_hidePopup('confirm-popup', 0);
							_showPopup('success-confirm-popup');
							_hidePopup('success-confirm-popup', 10000);
						} 
	...
	function _withdraw() {
		$('.btn-withdraw-farm-btcb-bnb-lp').click(function(e) {
			e.preventDefault();
			let amount = $('input[type=number][name=farm_btcb_bnb_lp_pool_withdraw_btcb_bnb_lp_amt]').val();
			amount = parseFloat(amount);
			// amount = toBN(amount, 18);
			let userAddr = getCurrentAddress();
			if (!userAddr) {
			...
			_contract
	            ...
	            .on('confirmation', function(confirmationNumber, receipt){
	                if (receipt.status == true && !_transactionHistory[receipt.transactionHash]) {
							_transactionHistory[receipt.transactionHash] = true;
						$('input[type=number][name=farm_btcb_bnb_lp_pool_withdraw_btcb_bnb_lp_amt]').val('');
						_hidePopup('confirm-popup', 0);
						_showPopup('success-confirm-popup');
						_hidePopup('success-confirm-popup', 10000);
					} 
