$(document).ready(function() {
    initWeb3();
    initFarmPoolV2Contract();
    try {
    	loadFarmPoolV2Data();
    } catch(e) {
    	reloadFarmPoolV2Data();
    }
    
    initUserAction();
});

function getWeb3ToReadDataForFarmCakeV2() {
...
function initFarmPoolV2Contract() {
	if (!farmCakePoolV2Contract) {
		const _web3 = getWeb3ToReadDataForFarmCakeV2();
...
function reloadFarmPoolV2Data() {
	setTimeout(function(){ 
		loadFarmPoolV2Data();
	}, 3000);
}
function loadFarmPoolV2Data() {
	if (!farmCakePoolV2Contract) {
		return reloadFarmPoolV2Data();
	}
	...
	function _updateUserData(_result) {
		...
		userDataInFarmCakePoolV2.miningSpeed = parseInt(_result[0]); 
		userDataInFarmCakePoolV2.userWantBal = parseFloatNumber(parseInt(_result[1]) / 1e18, 18);
		userDataInFarmCakePoolV2.turingPrice = parseFloatNumber(parseInt(_result[2]) / 1e18, 18);
		userDataInFarmCakePoolV2.wantPrice = parseFloatNumber(parseInt(_result[3]) / 1e18, 18);
		userDataInFarmCakePoolV2.totalMintPerDay = parseFloatNumber(parseInt(_result[4]) / 1e18, 18);
		userDataInFarmCakePoolV2.totalWantRewardPerDay = parseFloatNumber(parseInt(_result[5]) / 1e18, 18);
		userDataInFarmCakePoolV2.userBNBBal = parseFloatNumber(parseInt(_result[6]) / 1e18, 18);
		userDataInFarmCakePoolV2.userTuringPending = parseFloatNumber(parseInt(_result[7]) / 1e18, 18);
		userDataInFarmCakePoolV2.userWantShare = parseFloatNumber(parseInt(_result[8]) / 1e18, 18);
		userDataInFarmCakePoolV2.turingRewardAPY = 0;
		if (parseInt(_result[9]) > 0) {
			userDataInFarmCakePoolV2.turingRewardAPY = parseFloatNumber(parseInt(_result[9]) / 1e2, 2);
		}
		if (parseInt(_result[10]) > 0) {
			userDataInFarmCakePoolV2.wantRewardAPY = parseFloatNumber(parseInt(_result[10]) / 1e2, 2);
		}
		userDataInFarmCakePoolV2.tvl = parseFloatNumber(parseInt(_result[11]) / 1e18, 18);
		_drawUI();
	}	
	function _drawUI() {
		$('.farm-cake-pool-your-stake').html(formatBalance(userDataInFarmCakePoolV2.userWantShare, 6));
		$('.farm-cake-pool-your-earned').html(formatBalance(userDataInFarmCakePoolV2.userTuringPending, 6));
		$('.farm-cake-pool-total-supply').html(`$${formatBalance(userDataInFarmCakePoolV2.tvl * userDataInFarmCakePoolV2.wantPrice, 2)}`);
		$('.farm-cake-pool-user-cake-bal').html(numberWithCommas(userDataInFarmCakePoolV2.userWantBal, 6));
		$('.farm-cake-pool-user-cake-state').html(numberWithCommas(userDataInFarmCakePoolV2.userWantShare, 6));
		$('.farm-cake-pool-apy').html(`${formatBalance((userDataInFarmCakePoolV2.wantRewardAPY + userDataInFarmCakePoolV2.turingRewardAPY), 2)}%`);
		$('.farm-cake-pool-apy-cake').html(`${formatBalance((userDataInFarmCakePoolV2.wantRewardAPY), 2)}%`);
		$('.farm-cake-pool-turing-apy').html(`${formatBalance((userDataInFarmCakePoolV2.turingRewardAPY), 2)}%`);
...
function initUserAction() {
	...
	function _clickMaxDeposit() {
		$('.farm-cake-pool-max-cake-deposit').click(function(e) {
			...
			let amount = userDataInFarmCakePoolV2.userWantBal ? userDataInFarmCakePoolV2.userWantBal : 0;
	...
	function _clickMaxWithdraw() {
		$('.farm-cake-pool-max-cake-withdraw').click(function(e) {
			...
			let amount = userDataInFarmCakePoolV2.userWantShare ? userDataInFarmCakePoolV2.userWantShare : 0;
	...
	function _approve() {
		$('.btn-approve-farm-cake').click(function(e) {
			...
			_token
				.methods
				.approve(FARM_CAKE_POOL_V2_CONTRACT_ADDR, ALLOW_LIMIT_AMT)
	...
	function _deposit() {
		$('.btn-deposit-farm-cake').click(function(e) {
			...
			let farmPoolContract = new web3.eth.Contract(FARM_CAKE_POOL_CONTRACT_ABI, FARM_CAKE_POOL_V2_CONTRACT_ADDR);

			cakeToken
				.methods
				.allowance(userAddr, FARM_CAKE_POOL_V2_CONTRACT_ADDR)
				.call()
				.then(amountAllow => {
			...
			function _approvelToken() {
				...
				cakeToken.methods.approve(FARM_CAKE_POOL_V2_CONTRACT_ADDR, ALLOW_LIMIT_AMT).send({ from: getCurrentAddress() })
					.on('transactionHash', function(hash) {
	                	_showPopup('confirm-popup');
	                })
	                .on('confirmation', function(confirmationNumber, receipt){
