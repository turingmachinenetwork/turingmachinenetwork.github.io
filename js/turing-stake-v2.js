$.TuringStakeV2 = function () {
};

$.TuringStakeV2.prototype = (function () {
    ...
    var turingStakeContractAddr = {
        "0": "0x53a648B6A77AFc1A46815588809Ea8A6bD4a23c5",
        "1": "0x18bcd760Fa77BBE3Cf35A3004A482c9BC9Be3328",
        "2": "0x2403C811206d27eb40a81450d7125808B565F889"
    };
    ...
    return {
            ...
            Promise
                    ...
                    .then(r => {
                        ...
                        if(page != null && (page === "Dashboard" || page === "Home")) {
                            if (arrayTVLs) {
                                arrayTVLs["turingStakeTVLV2"] = totalStake * _price;
                            }
                            if (arrayUserTuringReward) {
                                arrayUserTuringReward["turingStakeUserRewardV2"] = _totalEarnedOfUser;
                            }
                            if (arrayUserBalance) {
                                arrayUserBalance["turingStakeUserBalanceV2"] = _totalStakeOfUser * _price;
                            }
                        }
                        if (user["0"]) {
        ...
        initInterface() {
            $(`.${setting.pool}-tvl-v2`).html("$"+core.formatBalance(totalTVL, 6));
            $(`.${setting.pool}-apy-v2`).html(core.formatBalance(totalAPY / totalPool, 2));

            if (page != null && page === "Dashboard") {
                if (totalStakeOfUser <= 0) {
                    $(`.${setting.pool}`).hide();
                } else {
                    $(`.${setting.pool}-user-balance-v2`).html(core.formatBalance(totalStakeOfUser, 2));
                    $(`.${setting.pool}`).show();
                }
            }
        },
