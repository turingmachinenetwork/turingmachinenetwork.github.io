$.TURRING_VENUS_POOL_REMOVE = function() {}
$.TURRING_VENUS_POOL_REMOVE.prototype = (function() {
    var setting = {
        chainId: 56
    };
    var transactions = {};
    ...
    return {
        ...
        getDataToRemoveLP() {
            ...
            try {
                ...
                let callContract = this._getTuringVenusSwapContractToReedData(contractName)
                if (!callContract) {
                    return false;
                }
        ...
        getLpBal() {
            return new Promise((resovel, reject) => {
                ...
                let callContract = this._getTuringVenusSwapContractToReedData(lp.contract)
        ...
        removeLp() {
            ...
            $('#remove-liquidity').click((e) => {
                e.preventDefault();
                let userAddr = coreHelper.getCurrentAddress();
                console.log("address user: ", userAddr);
                if (!userAddr) {
                    return false;
                }
                let _pool = $('select[name=lp_token]').val();
                ...
                console.log("contract address: ", _contractAddr);
                if (!_contractAddr) {
                    return false;
                }
                let _amountOfLiquidity = parseFloat($('input[name=amount_of_liquidity]').val()) / 100;
